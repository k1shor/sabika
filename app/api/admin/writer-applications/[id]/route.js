...
export async function PATCH(req, { params }) {
  ...
  await Notification.create({
    userId: user._id,
    writerId: user._id,
    type: parsed.data.status === "approved" ? "writer_approved" : "writer_rejected",
    message:
      parsed.data.status === "approved"
        ? "Your writer application was approved. You can post now from My Posts."
        : `Your writer application was rejected.${user.writerVerification?.rejectionReason ? ` Reason: ${user.writerVerification.rejectionReason}` : ""}`,
    read: false,
  });

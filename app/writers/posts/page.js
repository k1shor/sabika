...
export default function WriterPostsPage() {
  ...
  const loadPosts = async () => {
    ...
    const res  = await fetch("/api/writer/posts", { cache: "no-store" });
    const data = await res.json().catch(() => null);

    if (!data?.ok) {
      const code = data?.code;
      // Redirect away based on why they're blocked
  ...
  const createPost = async (e) => {
    ...
    const payload = {
      title, slug, excerpt, coverImage, contentHtml, category,
  ...
  const deletePost = async (slug) => {
    ...
    setMessage(null);
    setError(null);
    const res  = await fetch(`/api/writer/posts/${slug}`, { method: "DELETE" });
    const data = await res.json().catch(() => null);
    setBusy(false);
  ...
  // Still loading / redirecting — show skeleton
  if (loading) return (
    <Container>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/70 p-6 shadow-sm dark:border-blue-400/20 dark:bg-blue-950/25">
            ...
            <form onSubmit={createPost} className="mt-5 grid gap-4">
              ...
              <div>
                ...
                <div className="mt-2">
                  <TextArea name="excerpt" placeholder="Short summary shown in blog cards" hasError={!!fieldErrors?.excerpt} />
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-700 dark:text-blue-100/80">Post anonymously</p>
                  <p className="text-xs text-slate-500 dark:text-blue-100/50">Your name will be hidden. Only your role will be shown.</p>
                </div>
              </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/70 p-6 shadow-sm dark:border-blue-400/20 dark:bg-blue-950/25">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Your Posts</h2>
              ...
              <div className="mt-5 grid gap-3">
                {posts.map((post) => (
                  ...
                  >
                    ...
                    {post.excerpt && (
                      <p className="mt-2 text-xs text-slate-500 line-clamp-2 dark:text-blue-100/50">{post.excerpt}</p>
                    )}
                  </div>
                ))}
              </div>

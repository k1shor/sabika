    ...
    <div className="grid gap-6">
      ...
      >
        ...
        <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isPreview && followButton}
            {isLoggedIn && !isOwner && !isPreview && writer?._id && (
              <ReportButton targetType="writer" targetId={writer._id} />
            )}
          </div>
        </div>
      ...
      >
        <h2 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">
          {isPreview ? "Your Published Blogs" : "Writer Posts"}
        </h2>

        {posts.length === 0 ? (
          <div className="mt-5 text-sm font-semibold text-slate-600 dark:text-blue-100/70">
            {isPreview ? "You haven't published any blogs yet. When you do, they will appear here beautifully structured." : "No posts from this writer yet."}
          </div>
        ) : (
          <div className="mt-5 grid gap-3">
            {posts.map((post, i) => (
              ...
              >
                {isPreview ? (
                  <div className="block rounded-2xl border border-slate-200 bg-white/80 p-4 transition dark:border-blue-400/20 dark:bg-blue-950/30">
